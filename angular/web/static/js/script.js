...
$(document).ready(function(){
	...
	$('#wall').facebookWall({
		id:'160119364134350',//'smashmag'
		access_token: '520695387994024|NjcKEjDrJMUmA8G3KCyeKXSXon8'//'191042970944652|NGJyz1d0Kclt2XO3AyrjHmrKOHo'
	});
	...
	$.fn.facebookWall = function(options){
		...
		$.when($.getJSON(graphUSER),$.getJSON(graphPOSTS)).done(function(user,posts){
			...
			// Generating the feed template and appending:
                        p ='<li><div class="status"><img src="${from.picture}" class="avatar" /><h2><a href="http://www.facebook.com/profile.php?id=${from.id}" target="_blank">${from.name}</a></h2><p class="message">{{html message}}</p></div></li>'
                       	$(p).tmpl(fb.posts).appendTo(ul);
		});
